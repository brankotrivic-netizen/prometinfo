...
async function main() {
  ...
  for (const it of items) {
    ...
    const stale = dirAges.some((age) => age > 90) ? " · ⚠ vsaj ena smer je starejša od 90 min" : "";
    it.rawStatus = `🇭🇷 HAK/MUP: ${parts.join(", ")}${stale}`;
...
(function(){
  ...
  // HAK ima ločen čas meritve za vsako smer. Za trenutno pot vedno preveri
  // izbrano smer; podatek, starejši od 90 minut, ni več trenutna ocena.
  ...
  function reliab(p){
    ...
    if(age<=90) return {dot:'🟡', txt:'star 30–90 min'};
    return {dot:'🔴', txt:'HAK star > 90 min — ni uporabljen kot trenutno stanje'};
  }
  // ---- VEČ-VIRNI SLOJ: kateri vir pravi kaj ----
  function sourcesFor(p){
    var s=[];
    if(p.hak){ var hi=hakDirIso(p),a=hi?Math.round((Date.now()-Date.parse(hi))/60000):null; s.push({key:'hak',label:'🇭🇷 HAK/MUP',official:true,wait:true,ageMin:a}); }
  ...
  function paxTruck(p){
    ...
    if(p.hak){
      ...
      // Stare HAK številke so zgodovinski podatek, ne trenutno stanje.
      if(px!=null && hakDirStale(p)) px=null;
      // Če HAK nima svežega podatka za to smer, uporabi le BIHAMK-ovo
      // obojesmerno zgornjo mejo; nasprotne smeri nikoli ne prenašaj.
  ...
  function dirWaits(p){
    if(p.hak){
      ...
      function ageMark(iso){ var a=iso?Math.round((Date.now()-Date.parse(iso))/60000):null; return a!=null&&a>90?' <span class="meta" style="color:#dc2626">(star '+a+' min — ni trenutna ocena)</span>':''; }
      var um=ageMark(p.hak.ulazTsISO||p.hak.tsISO), im=ageMark(p.hak.izlazTsISO||p.hak.tsISO);
      // smer potovanja: naprej (od doma) = vstop v tujino (izlaz iz HR); nazaj = vstop v HR (ulaz)
      return '<div class="dirrow'+(REV?' rdir-hl':'')+'">'+(FLAGJS[other]||'')+'→🇭🇷 vstop v '+acc('HR')+': <b>'+u+'</b>'+um+(REV?MARK:'')+'</div>'
           + '<div class="dirrow'+(!REV?' rdir-hl':'')+'">🇭🇷→'+(FLAGJS[other]||'')+' vstop v '+acc(other)+': <b>'+iz+'</b>'+im+(!REV?MARK:'')+'</div>';
    }
    if(p.amss){
